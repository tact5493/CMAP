...
from src.utils.load_experiment_data import load_experiment_data
from src.utils.prepare_time_arrays import prepare_time_arrays
from src.utils.setup_pf_coil import setup_pf_coil
...
if __name__=="__main__":
    ...
    setup_directories(path)  # Create directories

    s, t_ip, ip, t_inj, inj, t_g, G_array, t_puc, bz_puc = load_experiment_data(count)
    t_inj_0, t_decay = prepare_time_arrays(t_inj, inj, t_ip, ip)  # Prepare time arrays
...
#. == Time loop start ==
    for t_ana in tqdm(t_ana_arr, leave = False, desc = "time"):
        ...
        #. wq -> Grid の容器壁判定
        wq = np.loadtxt(DATA_DIR / "ele_posi.csv", delimiter = ",")
        mfile_bin_path = DATA_DIR / "mfile_py.bin"
        
        #. Generate or Read the binary file mfile.bin
        A_0 = np.zeros((ir_max+1, iz_max+1, ir_max+1, iz_max+1))
        A_0 = read_or_generate_A0(mfile_bin_path, ir_max, iz_max, r, z, dr, dz)            
    ...
    #. == Calculating loop ==
        ...
        #. -- Caluculate Toroidal current from Lambda value --
            ...
            if Itor_cal_inp > 0:
                if (
                    (2*np.abs(I_tor[-1]) * lambda_fac >  Itor_cal_inp)
        ...
        #. -- Save text and graph --
            ...
            if update_con:
                ...
                if save_con:
                    ...
                    m_in = cal_flux(ir_in+1, iz_in+1)
                    save_and_plot_results(
                        i, t_ana, path, time_path, mf_output, jf_array, mv_flux,
                        Bz_cal_puc, Bz_vac_puc, z_puc, bz_puc_it, sq_error, I_tor,
                        ele_lim, dr, dz, r, A_phi
                    )
